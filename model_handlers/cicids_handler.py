...
class CICIDSHandler:
    def __init__(self):
        self.model_dir = Path("models/cicids")
    ...
    def load_model(self):
        try:
            ...
            # Load feature names - parse as JSON array
            feature_file = self.model_dir / "feature_names.json"
            if feature_file.exists():
                with open(feature_file, "r") as f:
                    self.feature_names = json.load(f)
            else:
                # Fallback to selected_features.json if feature_names.json doesn't exist
                selected_file = self.model_dir / "selected_features.json"
                with open(selected_file, "r") as f:
                    self.feature_names = json.load(f)
            
            # Load label classes
            self.labels = np.load(self.model_dir / "label_classes.npy", allow_pickle=True)
            
            #print(f"Loaded CICIDS model with {len(self.feature_names)} features")
            #print(f"Label classes: {self.labels}")
            #print("self.feature_names==", self.feature_names)
            
        except Exception as e:
            raise Exception(f"Failed to load CICIDS model: {str(e)}")
        ...
        try:
            ...
            if hasattr(self.pipe, 'steps'):
                ...
                if 'tree' in model_type or 'forest' in model_type or 'gbm' in model_type:
                    ...
                    print("Initialized TreeExplainer for CICIDS")
                else:
                    # For other models, use KernelExplainer with background data
                    # Create background data that matches the transformed feature space
                    background_data = pd.DataFrame(np.zeros((5, len(self.feature_names))), 
                                                 columns=self.feature_names)
                    # Transform through pipeline steps (except the last estimator)
                    background_transformed = self.pipe[:-1].transform(background_data)
                    self.explainer = shap.KernelExplainer(final_estimator.predict_proba, 
                                                        background_transformed)
                    print("Initialized KernelExplainer for CICIDS")
            else:
                # If no pipeline steps, use the model directly
                self.explainer = shap.TreeExplainer(self.pipe)
                print("Initialized TreeExplainer for CICIDS (direct model)")
    ...
    def create_shap_plot(self, df_input, prediction):
        ...
        try:
            ...
            print(f"Sample SHAP values shape: {sample_shap_values.shape}")
            print(f"Number of SHAP values: {len(sample_shap_values)}")
            ...
            filename = f"shap_cicids_{timestamp}_{unique_id}.png"
    ...
    def create_simple_shap_plot(self, df_input, prediction):
        ...
        try:
            print("Using fallback SHAP plot method for CICIDS")
            ...
            if hasattr(final_estimator, 'feature_importances_'):
                ...
                print(f"Using dummy importances, length: {len(importances)}")
            ...
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            unique_id = str(uuid.uuid4())[:8]
            filename = f"shap_cicids_fallback_{timestamp}_{unique_id}.png"
            filepath = self.shap_dir / filename
    ...
    def cleanup_old_shap_files(self, keep_count=20):
        ...
        try:
            shap_files = list(self.shap_dir.glob("shap_cicids_*.png"))
    ...
    def predict(self, form_data):
        try:
            # Prepare input
            input_values = []
            print(f"Processing CICIDS form data with {len(self.feature_names)} features")
            ...
            for feature in self.feature_names:
                ...
                elif f"cicids_{feature}" in form_data:
                    value = float(form_data.get(f"cicids_{feature}", 0))
            ...
            df_input = pd.DataFrame([input_values], columns=self.feature_names)
            
            # Predict
            prediction = self.pipe.predict(df_input)[0]
            prediction_proba = self.pipe.predict_proba(df_input)[0]
            confidence = max(prediction_proba)
            result = str(self.labels[int(prediction)])
            ...
            print(f"Returning CICIDS result with SHAP plot: {shap_plot_path is not None}")
            return result_data
